...
if len(sys.argv) > 1 and sys.argv[1] in ('-c', '--copy'):
    if len(sys.argv) > 2:
        pyperclipfix.copy(sys.argv[2])
    else:
        pyperclipfix.copy(sys.stdin.read())
elif len(sys.argv) > 1 and sys.argv[1] in ('-p', '--paste'):
    sys.stdout.write(pyperclipfix.paste())
else:
    print('Usage: python -m pyperclipfix [-c | --copy] [text_to_copy] | [-p | --paste]')
    print()
